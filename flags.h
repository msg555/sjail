...
REGISTER_FLAG(threads, 0, int, 0, "Global limit on additional thread creation. "
              "Negative limits indicate no limit. Default is 0.");
REGISTER_FLAG(exec_match, 0, string, "", "Valid filenames to pass to exec.  An "
              "empty string is default and interpreted as disabling exec.");
...
REGISTER_FLAG(listen, 0, bool, false, "Allows program to bind to a port. "
              "Requires --net.")
REGISTER_FLAG(net_regexp, 0, string, "", "Restricts tcp connections and udp "
              "outgoing packets to those where \"ip;port;{INET,INET6,UNIX}\" "
              "matches the given regexp.  Default is blank (match anything)")

#ifdef FLAG_SECTION
FLAG_SECTION("File options")
#endif
REGISTER_FLAG(files, 1, string, "", "Enable file read/write access to the "
...
REGISTER_FLAG(report_file, 0, string, "jail.out", "Sets the report file name. "
              "Default is jail.out.")
